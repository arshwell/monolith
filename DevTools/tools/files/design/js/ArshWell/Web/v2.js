/**
 * Web function (acting as a class) having some functions existent in PHP class Arsavinel\Arshwell\Web.
...
Web.query = function (str = null) {
    if (str == null) {
        str = window.location.search.substr(1)
    }
    if (!Web.vars['queries'].hasOwnProperty(str)) {
        Web.vars['queries'][str] = (function (str) {
    ...
    return Web.vars['queries'][str];
};

Web.site = function () {
    return ((window.location.protocol || location.protocol) + '//' + Web.vars['site']);
};

Web.statics = function (name, path = null) {
    return (location.protocol +"//"+ Web.vars['statics'][name] + (path || ""));
};
...
Web.url = function (key = null, values = null, page = 0, $_request = null) {
    var route = (key == null ? Web.vars['route'] : Web.vars['routes'][key]);
    ...
    if ($_request) {
        route.url += ("?"+ (typeof $_request === "string" ? ($_request+"&") : (Object.keys($_request).length ? (http_build_query($_request)+"&") : "")) + "mxdvcwdthflg="+ Math.max(
...
Web.go = function (url, container, pushState = true) {
