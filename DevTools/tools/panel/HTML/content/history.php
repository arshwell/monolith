<ul class="list-group">
    <?php
    foreach (array_reverse(Arsh\Core\Session::history()) as $index => $route) { ?>
                ...
                <div class="offset-3 offset-md-2 col">
                    <div class="collapse" id="history--<?= $index ?>">
                        <table class="table table-sm table-bordered table-dark m-0 mt-2">
                            ...
                            if (Arsh\Core\Web::exists($route['key'])) { ?>
                                <tr>
                                    <th class="w-25">URL</th>
                                    <td class="w-75 break-word"><?= Arsh\Core\Web::url($route['key'], $route['params'], $route['language'], $route['page'], $route['$_GET']) ?></td>
                                </tr>
                            <?php }
