...
if ($form->valid()) {
    ...
    foreach (File::rFolder(StaticHandler::getEnvConfig()->getFileStoragePathByIndex(0, 'uploads') . 'files/') as $file) {
        if (is_file($file) && ($matches = File::parsePath($file))) {
            ...
            // class doesn't exist || table row doesn't exist || class don't have files
                ...
                $count = count(File::rFolder(StaticHandler::getEnvConfig()->getFileStoragePathByIndex(0, 'uploads') . 'files/' . $path));

                if (Folder::remove(StaticHandler::getEnvConfig()->getFileStoragePathByIndex(0, 'uploads') . 'files/' . $path)) {
                    $removed[0] += $count;
            ...
            if (!is_subclass_of($class, TableView::class)) {
                Folder::removeEmpty(StaticHandler::getEnvConfig()->getFileStoragePathByIndex(0, 'uploads') . 'files/' . $matches['class']);
            }
        }
    }
