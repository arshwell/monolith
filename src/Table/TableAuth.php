...
abstract class TableAuth extends Table {
    ...
    final static function setCookieID (int $id, int $expires): bool {
        return setcookie("Arshwell[".static::class."]", $id, time() + $expires, StaticHandler::getEnvConfig()->getSiteRoot() ?: '/');
    }
    // object
    final function setCookie (int $expires): bool {
        return setcookie("Arshwell[".static::class."]", $this->id_table, time() + $expires, StaticHandler::getEnvConfig()->getSiteRoot() ?: '/');
    }
